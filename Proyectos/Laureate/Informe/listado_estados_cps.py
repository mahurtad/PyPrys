...
# Filtrar los casos de prueba que pertenecen a GL1 y GL3
df_gl1_gl3 = df[(df['N° GL'] == 1) | (df['N° GL'] == 3)]
...
df_gl1_gl3['Estado de CP'] = df_gl1_gl3.apply(asignar_estado, axis=1)
...
df_reporte = df_gl1_gl3.groupby(['N° GL', 'N° CP']).apply(determinar_estado_agrupado).reset_index(name='Estado de CP')

# Concatenar valores únicos de otros campos para el reporte final
df_reporte = df_reporte.merge(
    df_gl1_gl3.groupby(['N° GL', 'N° CP']).agg({
        'N° D': lambda x: ', '.join(x.dropna().astype(str).unique()),
        'Caso de Prueba': lambda x: ', '.join(x.dropna().unique()),  # Concatenación de descripciones de "Caso de Prueba"
...
columnas_seleccionadas = ['N° GL', 'N° CP', 'N° D', 'Caso de Prueba', 'Nombre de Tarea (Desarrollo)', 'BPO', 'PM Asignado', 'Periférico', 'Estado de CP']
df_reporte = df_reporte[columnas_seleccionadas]

# Exportar los resultados a un archivo Excel
output_path = os.path.join(os.path.dirname(file_path_new), 'matriz_principal_cps_concatenado_06022025.xlsx')
