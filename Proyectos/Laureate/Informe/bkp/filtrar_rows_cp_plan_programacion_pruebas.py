...
sheet_name = 'Programación Pruebas'
column_name = 'Fecha'  
...
if not filtered_rows.empty:
    ...
    columns_to_display = ['Fecha', 'BPO', 'PM Asignado', 'Periférico', 'Estado CU', 'Estado Planificación', 'Hora de Inicio', 'Hora de Fin', 'Hora de Inicio', 'QA Lider Prueba','QA Crea Informe']
    ...
    print(f"\nFilas que contienen la fecha {filter_date.strftime('%d/%m/%Y')} según 'Fecha Caso Uso':\n")
    print(tabulate(filtered_rows, headers='keys', tablefmt='grid', showindex=False))
else:
    print(f"No se encontraron filas con la fecha {filter_date.strftime('%d/%m/%Y')} en 'Fecha Caso Uso'.")
