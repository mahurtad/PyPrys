...
for folder_path in folder_paths:
    for folder_name in os.listdir(folder_path):
        ...
        if os.path.isdir(full_path):
            ...
            chg = ""
            search_value = sctask if sctask else third_segment
            if search_value:
                chg_values = tickets_df.loc[
                    tickets_df["Short description"].str.contains(search_value, na=False, case=False, regex=False),
                    "Number"
                ].values
            ...
            # Buscar "NUM." en la hoja "Tickets Diarios" usando SCTASK
            num = certificaciones_df.loc[certificaciones_df["SCTASK"] == sctask, "NUM."].values
...
# Aplicar formato al archivo exportado
...
# Aplicar estilo de fecha a columnas Created y Updated (I y J)
for row in ws.iter_rows(min_row=2, max_row=ws.max_row, min_col=9, max_col=10):
    for cell in row:
        cell.style = date_style

# Estilo de fondo para Finalizados
