...
COLUMNS_CANDIDATES = {
    ...
    "estatus_implementador": [
        "estatus implementador"
    ]
}
...
def main():
    ...
    col_impl    = find_column(df, COLUMNS_CANDIDATES["implementador"])
    col_est_impl = find_column(df, COLUMNS_CANDIDATES["estatus_implementador"])

    missing = [name for name, col in [("Carrera del plan", col_carrera),
                                      ("Nombre del curso", col_curso),
                                      ("Estatus consultora", col_estatus),
                                      ("Implementador", col_impl),
                                      ("Estatus Implementador", col_est_impl)] if col is None]
    if missing:
    ...
    df["_implementador_"] = df[col_impl].astype(str).str.strip().str.lower()
    df["_estatus_impl_"] = df[col_est_impl].astype(str).str.strip().str.lower()

    # FILTRO: Implementador = "Manuel" y Estatus Implementador = "Pendiente"
    df = df[(df["_implementador_"] == "manuel") & (df["_estatus_impl_"] == "pendiente")]
