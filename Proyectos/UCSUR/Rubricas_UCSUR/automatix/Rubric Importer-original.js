  ...
  function blockRubric(txt) {
    ...
    try {
      ...
      while (isValid && i < lines.length) {
        ...
        var outcome = false;
        var name = words[0]; // Título del criterio
        var longDesc = ''; // Descripción larga (opcional)
  
        // Extraer puntajes, títulos y descripciones
        var descriptions = [];
        var points = [];
        for (var j = 1; j < words.length; j += 3) {
          if (words[j] && words[j + 1] && words[j + 2]) {
            var point = words[j]; // Puntaje de la calificación
            var title = words[j + 1]; // Título de la calificación
            var description = words[j + 2]; // Descripción de la calificación
  
            if (isPoints(point)) {
              points.push(point);
              descriptions.push({
                'title': title, // Título de la calificación
                'description': description // Descripción de la calificación
              });
            } else {
              errors.push('Second item in pair is not a number in line ' + i + ', column ' + (j + 1));
              isValid = false;
            }
          } else {
            errors.push('Missing data in line ' + i + ', columns ' + j + ' to ' + (j + 2));
            isValid = false;
          }
        }
        ...
        if (isValid) {
          var item = {
            'name': name, // Título del criterio
            'longDesc': longDesc, // Descripción larga (opcional)
            'outcome': outcome, // Resultado de aprendizaje (opcional)
            'ratings': descriptions, // Calificaciones (título y descripción)
            'points': points // Puntajes
          };
