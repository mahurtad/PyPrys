  ...
  function blockRubric(txt) {
    ...
    try {
      ...
      while (isValid && i < lines.length) {
        ...
        for (var j = 1; j < words.length; j += 3) {
          if (words[j] && words[j + 1] && words[j + 2]) {
            ...
            if (isPoints(point)) {
              points.push(point);
              descriptions.push({
                'title': formatText(title, 'bold'), // Título en negrita
                'description': description // Descripción sin formato
              });
            } else {
        ...
        if (isValid) {
          var item = {
            'name': formatText(name, 'bold'), // Título del criterio en negrita
            'longDesc': longDesc,
            'outcome': outcome,
