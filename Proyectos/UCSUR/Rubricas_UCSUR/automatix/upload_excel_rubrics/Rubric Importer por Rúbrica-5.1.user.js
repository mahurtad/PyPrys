// ==UserScript==
// @name        Rubric Importer por Rúbrica
...
(function() {
  ...
  function processDialog() {
    ...
    if (errors.length === 0) {
      criteria = parseRubric(txt) || blockRubric(txt) || flexRubric(txt);
  ...
  function parseRubric(txt) {
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        // Extraer el nombre del criterio (primer campo)
        var criterionName = parts[0];

        var criterion = {
            'description': criterionName, // Nombre del criterio (e.g., "Descripción de la experiencia")
            'ratings': []
        };
        ...
        for (var j = 1; j < parts.length; j += 3) {
            ...
            // Extraer el título y la descripción de la calificación
            var ratingTitle = parts[j + 1]; // Título de la calificación (e.g., "Logro Destacado")
            var ratingDescription = parts[j + 2]; // Descripción de la calificación

            criterion.ratings.push({
                'description': ratingTitle, // Solo el título (e.g., "Logro Destacado")
                'points': points,
                'long_description': ratingDescription // Descripción detallada
            });
        }
