...
(function() {
  ...
  // Nueva función para aplicar formato
  function formatText(text, type) {
    if (type === 'bold') {
      return '<b>' + text + '</b>'; // Aplica negrita
    }
    return text; // Sin formato para descripciones
  }
  ...
  function addCriterion(item) {
    ...
    var name = formatText(item.name.replace(/\s*\\n\s*/g, ' ').replace(/\s+/g, ' '), 'bold'); // Título en negrita
  ...
  function addRatings(descriptions, points) {
    ...
    for (var i = 0; i < n; i++) {
      ...
      ratings.push({
        ...
        'points' : formatText(points[j], 'bold') // Puntaje en negrita
      });
    }
    return ratings;
  }
