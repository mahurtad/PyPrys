...
def es_fila_resumen(*textos):
    """Detecta si una fila representa rangos tipo '16–20 ptos.' o similares"""
    return all(
        re.search(r"\d+\s*[-–]\s*\d+\s*pto?s?\.?", texto.lower()) or "total" in texto.lower()
        for texto in textos
    )
...
def procesar_documento(word_file_path):
    try:
        ...
        # Validación: suma total de Logro Destacado debe ser 20
