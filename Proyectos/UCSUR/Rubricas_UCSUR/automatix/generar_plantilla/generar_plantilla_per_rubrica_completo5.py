...
# === FUNCIONES DE PARSING PARA NUEVO FORMATO ===
def normalizar_puntaje(valor):
    if isinstance(valor, str):
        valor = valor.strip().replace(",", ".")
        # Manejar casos como "(5)" -> 5
...
def extraer_puntaje_y_descripcion_nuevo_formato(texto):
    """
    Versión adaptada para el nuevo formato donde el puntaje está al final entre paréntesis
    Ejemplo: "Descripción del criterio (5)"
    """
    texto = texto.strip()
    
    # Buscar puntaje al final entre paréntesis
...
# === PROCESAMIENTO DE DOCUMENTO WORD - ADAPTADO ===
        ...
        for table in doc.tables:
            ...
            for row in table.rows:
                ...
                if celda_0 and not any(x in celda_0.upper() for x in ["PTS", "PUNTOS", "TOTAL", "NOTA"]):
                    ...
                    if p_ld is None:
                        p_ld = extraer_puntaje_desde_texto(celda_1)
                    if p_l is None:
                        p_l = extraer_puntaje_desde_texto(celda_2)
                    if p_nl is None:
                        p_nl = extraer_puntaje_desde_texto(celda_3)
...
# === FUNCIONES ADICIONALES (se mantienen igual) ===
